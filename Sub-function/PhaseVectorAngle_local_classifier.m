function [classification_accuracy_avg,classification_accuracy_stderr] = PhaseVectorAngle_local_classifier(No_of_Subject,vx_math_listen_alltrials,vy_math_listen_alltrials,vx_math_answer_alltrials,vy_math_answer_alltrials)
disp(['initiating...'])    
...
for irow = 10:170
    ...
    for icol = 130:220
        ...
        vxy_math_listen_angle_ROI = angle(vx_math_listen_alltrials(y_array,x_array,:,:) + i.*vy_math_listen_alltrials(y_array,x_array,:,:));    
        vxy_math_answer_angle_ROI = angle(vx_math_answer_alltrials(y_array,x_array,:,:) + i.*vy_math_answer_alltrials(y_array,x_array,:,:));    
        ...
        temp1_math_listen_train_template_vx = nanmean(nanmean(vx_math_listen_alltrials(y_array,x_array,:,:),4),3);
        temp1_math_listen_train_template_vy = nanmean(nanmean(vy_math_listen_alltrials(y_array,x_array,:,:),4),3);
        ...
        % training template for math listening tasks                        
        temp1_math_answer_train_template_vx = nanmean(nanmean(vx_math_answer_alltrials(y_array,x_array,:,:),4),3);
        temp1_math_answer_train_template_vy = nanmean(nanmean(vy_math_answer_alltrials(y_array,x_array,:,:),4),3);
        ...
        for trial = 1:size(test_trial_math_listen,2)
        ...
        % for each math test trial, calculate its correlation coefficient with  
        % 2 training templates (math listening and answering)
        for trial = 1:size(test_trial_math_answer,2)
            % remove voxels in both test trials and training templates if 0 
            % and nan values are found in either 
            % this step is to ensure the same data size with no nan/0 in 
            % all variables when calculating correlation coefficients
